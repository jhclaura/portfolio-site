	...
	loader.load(model, function(geometry, material){
		...
		loader.load(model_B, function(geometryB, materialB){
			...
			for(var i=0; i<40; i++){
				bananaCore = new THREE.Mesh(geometryB, new THREE.MeshLambertMaterial({color: 0xffffff}));
				bananaCore.scale.set(0.5,0.5,0.5);
				bananaCore.position.set(Math.random()*50-25, -1, -2+Math.random()*46-23);
...
function myMouseMove(event) {
	...
	UpdateTargetCube(event.clientX, event.clientY);

	// vector.set( (event.clientX / window.innerWidth)*2-1, -((event.clientY+document.getElementsByTagName('body')[0].scrollTop) / (window.innerHeight/3))*2+1, 0.5 );
...
function UpdateTargetCube(xPos, yPos)
{
	vector.set( (xPos / window.innerWidth)*2-1, -((yPos+document.getElementsByTagName('body')[0].scrollTop) / (window.innerHeight/3))*2+1, 0.5 );
	vector.unproject(camera);
...
{		
	...
	if(!meClick && !meWalking){
		...
		var randomDestination = new THREE.Vector3(Math.random()*30-25, 0, -5+Math.random()*50-25);
